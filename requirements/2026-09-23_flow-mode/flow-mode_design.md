...
| 2 | 打字机形态是否算「心流偏好」 | 需求原表把「打字机形态」列在心流偏好（退出不改、下次沿用）。实现简化为**推导式**：心流内 `off → bottom`，用户选过则沿用自己的形态；不改写设置项，故无需快照还原。好处是四条轴（侧栏 / 专注 / 预览 / 打字机）语义完全一致，也少一个冗余设置项 |
...
**交付**：`lib/caretSound.ts`（Web Audio 合成器：限流与合成参数为纯函数，可单测）+ 设置项 `flowSoundEnabled` / `flowSoundVolume`（五处同步）+ 设置页开关与音量滑杆 + `tests/caretSound.test.ts`（6 项）。
...
**待人工确认**：音色观感（CDP 无法试听）。若需调整，改 `synthParams()` 的频率/增益、`metalComponent` 的回声参数、或 `ratchetSchedule` 的间隔与包络即可。
