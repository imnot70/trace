...
| 变量值 | 非空字符串、≤64 字符、匹配 `^[#a-zA-Z0-9(),.%\s/-]+$`（禁止 `; : { } < > " ' \`，阻断 CSS 声明逃逸与 `url()` 外链） |
...
- `save(theme)`：**再次校验**（防御）→ 写 `themes/<id>.json`（规范化内容，UTF-8）。
- `list()`：读取目录下 `*.json`，逐个解析；**无效文件静默跳过**，按 `name` 排序。
- `remove(id)`：删除对应文件；文件不存在也算成功。
