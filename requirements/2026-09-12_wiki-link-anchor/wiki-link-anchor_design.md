...
| 面板形态 | 编辑卡右下角悬浮胶囊入口 + 弹层（首版为占布局的底部折叠条，会把编辑区压得比左右卡片短，与三栏卡片布局冲突后改为悬浮式）；无引用时不渲染；`Esc` / 点击外部 / 切换笔记收起；弹层列表仅显示来源笔记标题（首版含引用片段，但点击跳转并不定位到引用点，片段失去上下文意义，已移除）；同一笔记的多次引用合并为一条；受设置 → 通用 → 编辑器 →「显示反向链接」开关控制 |
