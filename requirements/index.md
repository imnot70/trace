...
> 更新：2026-09-20 ｜ 发布基线：**v0.4.6**（Windows 透明窗口回归修复）；工作区干净
...
| [plugin-design.md](2026-09-08_plugin-system/plugin-design.md) | 插件系统 v2 设计：独立插件进程 + 能力网关 + require 白名单 + 市场分发 | ✅ **M1 + M2 + M3 已实施**（进程隔离 + Tier 1 API + 权限确认 + 崩溃守护 + .trace-plugin 导入导出 + 私有存储 + 声明式工具栏/状态区 + 类型包，实施记录见第 10/11/12 节；开发指引见 [guides/plugin-development.md](../../guides/plugin-development.md)）；M4 待做 |
...
- **透明窗口的底部圆角在浅色壁纸上仍显示为直角**（仅 Linux；Windows 已随透明窗口整体禁用而消除，Ubuntu 推测为系统合成器限制）：内容区底部两角已做 10px 圆角且窗口透明（圆角缺口透出壁纸），但浅色壁纸下肉眼仍可见直角轮廓，疑似系统侧（窗口投影 / 合成器）在窗口边界外仍有方形绘制，`hasShadow: false` 在部分环境未完全消除；深色壁纸下正常。待按显示环境（X11 / Wayland、合成器差异）进一步排查。
...
里程碑：**M1 进程隔离与能力网关（✅ 已实施，第 10 节）** → **M2 `.trace-plugin` 导入导出 / 设置页详情 / 私有存储（✅ 已实施，第 11 节）** → **M3 声明式 UI 扩展与类型包（✅ 已实施，第 12 节）** → M4 市场浏览 / 安装 / 更新。
...
- 插件完整 API 与市场（v2 设计已定稿，见 `requirements/2026-09-08_plugin-system/`；**M1 + M2 + M3 已实施**，M4 待做）
- ~~多窗口~~（已作废）
