...
- 无待发版分支、无规划中的新功能；工作区当前停在 main。

### 2. 待办清单（按建议顺序）

| # | 事项 | 位置 / 说明 |
| --- | --- | --- |
| 1 | **加固批次**（技术债评审发现）：① 冲突解决路径穿越（安全）；② 搜索结果打开笔记污染常用列表；③ 搜索索引不随文件变更更新；④ settings 白名单 / git 同步互斥 / watcher 补偿缺口 / JsonStore 版本化 | 第二节「技术评审发现」；改动都很小，建议随 v0.8.3 之后的下一个补丁版 |
...
> 本节所列四个分支均已合并进 main，并随 **v0.8.3**（2026-09-25）发布。
