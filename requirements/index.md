...
- **main** = v0.8.2 + 两个已合并分支（预览初始位置修复 e83976a、状态区 77c2483）；
- **`feat/flow-quick-reference`**（af65f30，当前分支）：心流快速查阅（补全面板 Alt+Enter 预览 + 预览内导航），**待验证后合并**；
- 三批待发版改动（上两批已合入 main，本分支待合）建议合并后发 **v0.8.3**（全是小改进，打一个包）。

### 2. 待办清单（按建议顺序）

| # | 事项 | 位置 / 说明 |
| --- | --- | --- |
| 1 | **真机验证三个待发版分支**：状态区图标与保存圆点观感、胶囊 hover 命中、Alt+Enter 手感、预览初始位置 | 本节 1；改完的都在真机过一遍再合 |
| 2 | **合并 `feat/flow-quick-reference` → main → 发 v0.8.3** | `npm version patch`（postversion 自动推标签触发 CI） |
...
- **验证脚本自身要自证正确**（探针漏传参数 / JSON 字符串当对象读，两次差点误导排查方向）。
...
### 待发版（分支 `feat/zen-status-area`，基于含预览修复的 main）
...
### 待发版（分支 `fix/preview-initial-position`）
