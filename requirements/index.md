...
> 更新：2026-09-24 ｜ 发布基线：**v0.8.2**；`feature/tag-improvements`（标签区折叠 FR-2.6.11 + 侧栏区块「+」按钮对齐 + 搜索按钮移至标题行右端 + v0.8.0 编辑器空格/数字键 P0 回归修复）与 `fix/flow-typewriter-anchor`（进心流后光标不停在锚点线）均已合并进 main 并发版。⚠️ v0.8.0 的 P0 缺陷（编辑器打不出空格与数字 0-9）**已随 v0.8.1 修复**。当前有两条待办：搜索支持标签维度（第四节「待增补」）、打字机留白的两个收尾缺陷（第二节「待打磨」）
