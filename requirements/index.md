...
- **透明窗口的底部圆角在浅色壁纸上仍显示为直角**（v0.4.4 待修）：内容区底部两角虽已做 10px 圆角且窗口透明（圆角缺口透出壁纸），但浅色壁纸下肉眼仍可见直角轮廓，疑似系统侧（窗口投影 / 合成器）在窗口边界外仍有方形绘制，`hasShadow: false` 在部分环境未完全消除；深色壁纸下正常。待按显示环境（X11 / Wayland、合成器差异）进一步排查。
