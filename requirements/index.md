...
- 顶栏隐藏时编辑卡右下角渲染长条形状态区（胶囊形、底色 + 描边、85% 不透明度），内容按写作关注度排序：**编辑模式图标**（所见即所得 = 魔法棒 accent 高亮 / 源码 = 代码图标，随 Ctrl+E 实时切换）→ 保存状态文字（未保存 / 保存中… / 已保存 / 保存失败红色；无事件时隐去）→ Git 状态（同步中转圈 / 分支 / ↑↓ / 未提交；次要，非所有库都用 git）；悬停唤出顶栏时淡出。
