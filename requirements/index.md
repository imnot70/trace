...
- **Git 网络代理配置**：设置 → 通用可配置 HTTP/HTTPS 代理（支持凭据），「测试连接」一键验证可达性；仅在 git 调用时注入，不写入 .git/config（09-08 Linux 机）
- 修复：专注浮层侧栏超出窗口底部
- （09-08 会话，Linux 机）修复侧栏笔记库按钮/下级菜单失效（el-tooltip 包裹交互元素的回归，tooltip 禁令见 AGENTS.md）、下拉关闭时左上角闪影（menu-hold 锚点保持）；移除侧栏全部 EP 悬浮提示
