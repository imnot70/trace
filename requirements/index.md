...
> 更新：2026-09-25 ｜ 发布基线：**v0.8.2**；待发版：`fix/preview-initial-position`（预览 / 悬浮预览初始位置修复，见第四节）。⚠️ 打字机留白的两个收尾缺陷的修复尝试（`fix/typewriter-cleanup`）已放弃并删除（引入了新问题，见第二节），缺陷本身**回到待修状态**。其余待办：搜索支持标签维度（第四节「待增补」）、音色打磨 / IME 实机验证 / 行尾字母折行（第二节「待打磨」）
...
| 4 | **搜索支持标签维度**（用户已明确要做） | 第四节「待增补」；形态待定（筛选下拉 / tag: 语法 / 并入匹配），立项时补 FR 编号 |
