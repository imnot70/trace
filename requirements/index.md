...
> 更新：2026-09-24 ｜ 发布基线：**v0.8.1**；`feature/tag-improvements` 已合并进 main 并发版（标签区折叠 FR-2.6.11 + 侧栏区块「+」按钮对齐 + 搜索按钮移至标题行右端 + v0.8.0 编辑器无法输入空格/数字的严重回归修复）；工作区在 `fix/flow-typewriter-anchor` 分支，有一项未发布修复（进心流后光标不停在锚点线，见第四节「待发版」）。⚠️ v0.8.0 的 P0 缺陷（编辑器打不出空格与数字 0-9）**已随 v0.8.1 修复**。另有一条用户提出的待办（搜索支持标签维度，见第四节「待增补」）
...
- **打字机留白的两个收尾缺陷**（2026-09-24 用户实测反馈，**待修**，见[设计文档 10.9](2026-09-23_flow-mode/flow-mode_design.md) 末尾「遗留（待办）」）：
...
### 待发版（`fix/flow-typewriter-anchor` 分支，2026-09-24）
...
- **实测**：字号 15→26（内容层高度变、容器 561×765 不变）光标从 0.512 漂到 **1.78**（视口外、4s 不回来）→ 修复后 **0.52**；进心流两态 0.811 / 0.511 ✓；**近一页（31 行）与短于视口（12 行）的笔记**同样 0.811 / 0.511 ✓；滚轮上滚后位置未被拉回 ✓；打字机生效时输入空格 / 数字 / 字母正常插入且回到锚点 ✓。
- **遗留**：用户原场景未逐字复现（本夹具下进心流修复前后均能锚定），上述「字号」用例是同类根因的可复现证据；若再出现需记录打字机设置值 / 笔记长度 / 进心流前是否滚动过 / 是否稳定复现。
