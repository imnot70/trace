...
- **预览增强三件套**（FR-2.4.1 升级 / FR-2.4.7 / FR-2.4.8）+ GFM 任务列表（FR-2.4.2 补充）：内嵌 HTML 支持（DOMPurify 净化 + CSP form-action 兜底）、链接跳转（外部走系统浏览器 + 库内笔记应用内打开 + 断链删除线）、行级双向同步滚动（data-source-line 行号映射，修复旧比例同步从未生效的缺陷）；渲染管道抽取 lib/markdown.ts，新增 8 项渲染净化单测（jsdom）
