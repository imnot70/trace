...
## 9. 缺陷修复（v0.8.0 发布后）：空格与数字 0-9 打不进编辑器

**现象**（2026-09-24 用户实测反馈）：笔记编辑区无法输入空格。排查发现数字 `0`-`9` 同样打不进去，字母正常。

**根因**：§4.2 的提示态键位给 `Space` / 数字 / `Enter` / `Escape` 都声明了 `preventDefault: true`，注释里却写着「未激活时一律放行」——这两者矛盾。CM6 的实际语义（`@codemirror/view` 的 `runHandlers` / `runFor`）：

- 命令**返回 `true`** → 事件分发里 `if (handler(view, event)) event.preventDefault()`，CM 自己阻断，**不需要显式声明**；
- 命令**返回 `false`** → 才轮到绑定上的 `preventDefault` 标志生效，它会把按键标记为已处理（`if (prevented) handled = true`）并阻断默认行为。

于是「未激活 → 命令返回 false」这一路径恰好踩中标志：空格与数字的浏览器默认行为（字符插入）被吞掉。字母之所以正常，是因为没有任何绑定匹配字母。`Enter` / `Escape` 之所以「看着正常」，是因为换行 / 取消都由 CM 自己的键位处理，不依赖浏览器默认行为。

**修法**：去掉这四个绑定（含 10 个数字键）上的 `preventDefault: true`，与同文件 `Tab` / `Shift-Tab` 的既有修法一致（见 `MarkdownEditor.vue` 内注释与 AGENTS.md 的键位教训条目）。

**实测（隔离实例 + 真实输入管线 `Input.dispatchKeyEvent`）**：

| 状态 | 按键 | 修复前 | 修复后 |
| --- | --- | --- | --- |
| 提示态未激活 | 空格 | 插入 0 字符（被 `preventDefault`） | **插入 1 字符** |
| 提示态未激活 | 数字 `5` | 插入 0 字符 | **插入 1 字符** |
| 提示态未激活 | 字母 `a`（对照） | 插入 1 字符 | 插入 1 字符 |
| 提示态未激活 | `Enter` | 换行正常（CM 自处理） | 换行正常 |
| 提示态激活 | 数字 / 空格 | 被浮层吃掉 | 仍被吃掉，**正文零写入** |
| 提示态激活 | 输入「5 空格 6」 | 浮层回显 5 行 × 6 列 | 浮层回显 5 行 × 6 列，按空格插入 6 列表格 |
| 提示态激活 | `Esc` | 取消不插入 | 取消不插入 |

**同类排查**：渲染进程其余 `preventDefault: true` 绑定全在 `Mod-*` 组合键上，命令均无条件返回 `true`，不受影响。
