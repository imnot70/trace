...
**预览 / 悬浮预览初始位置不正确**（用户实测反馈，分支 `fix/preview-initial-position` 待发版）：

- **现象**：编辑区滚动到中部后打开预览，预览停在文档开头（0%）；滚动一次后同步恢复正常。
- **根因**：3.2 节的同步机制完全由 scroll 事件驱动——预览初次挂载（`v-if` 从 false 变 true）时预览从 scrollTop=0 起步、编辑器也没有滚动，双向都收不到事件，不存在任何同步。
- **修复**：三条出现路径（分栏开关打开 / 悬浮预览呼出 / 打开笔记且预览可见）主动调用一次 `syncToLine(编辑器当前可视首行)`。
- **连带修复一（防回环 guard 的时序缺口）**：主动同步写入预览 `scrollTop` 本身会触发预览 scroll 事件 → 走「预览→编辑器」方向同步 → CM 滚动，实测把编辑器从 87% 拖到 2%（预览挂载初期布局未稳、`lineAtScrollTop` 读出的行号偏差大）。修法：同步前置位防回环 guard（`source='editor'`），让紧随的回环事件被既有 100ms 窗口丢弃。教训：**凡程序化写入被监听的滚动容器，都要先置位防回环 guard**（3.2 节原有的 guard 只保护「事件处理过程中」的写入，不覆盖「事件之外」的主动写入）。
- **连带修复二（`nextBlockHeight` 尾块回退）**：目标块是文档最后一个块时，块内比例折算固定按 200px 回退——长段落 / 长代码块（自身高达数千 px）场景下同步位置严重偏早（实测预览停在 4.6% 而非 ~87%）。现回退到块自身渲染高度（`offsetHeight`）。
- **实测**：119 个独立段落的长文档：编辑器第 62 行（57%）→ 开预览后预览首块源行 126（= 62×2+2，精确命中），编辑器位置不被拉回；悬浮预览同样生效（30% 呼出 → 首块源行 78）。
