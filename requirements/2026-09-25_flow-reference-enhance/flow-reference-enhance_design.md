...
1. **插入引用多出 `]` 且渲染为断链**：编辑器 closeBrackets 会在输入 `[[` 时自动补出成对 `]]`（光标在括号中间），「插入引用」只替换到光标、没吸收光标后那对自动闭合括号——产生 `[[x]]]` 三连括号，多余 `]` 被并进链接目标后渲染为断链（该瑕疵同样存在于 v0.8.3 起的「Enter 接受补全」路径）。修法：抽出 `applyNoteCompletion(view, from, to, label)` 统一三条落成路径（Enter 接受补全 / 预览态 Alt+Enter / 预览「插入引用」按钮）——替换补全范围时**吸收光标后紧邻的至多两个 `]`**（无自动闭合则补 `]]`，保持 v0.8.3 行为）。
...
3. **底色过渡（FR-2.9.8 扩展，用户提出）**：写作底色新增「底色过渡」开关——写作栏两侧各 10% 栏宽的渐隐带，底色向两侧渐变透明。实现：`linear-gradient` 画在滚动层（透明 → 底色 → 透明，五段 stop 与栏宽 padding 同口径），中段实心区被上方行号槽 / 内容层的实底覆盖（渐隐带与实底不可能出现接缝），实心区外扩 24px 吸收滚动条宽度差；设置页在「底色颜色」下新增「底色过渡」开关（`flowPaperFade`，默认关）。
