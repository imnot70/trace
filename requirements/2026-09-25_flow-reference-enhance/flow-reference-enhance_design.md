...
1. **括号语义按路径拆分**：三轮的「统一吸收」实现两个方向各错一半——Enter 接受路径（替换范围在 `[[` 之后）吸收删除了 closeBrackets 的闭合括号又不补回（`[[label` 缺右括号）；预览态插入路径（替换范围含 `[[` 起点）吸收后只插裸名字（前后都没了 `[]`）。现拆分为各自语义：接受路径**保留既有闭合、仅缺失时补 `]]`**；插入路径**整段替换为 `[[路径]]` 并吸收残留 `]]`**。以磁盘文件内容为断言依据（所见即所得渲染会隐藏括号，DOM 断言不可靠）。
2. **底色过渡调整**：宽度 10% → **15%**；接缝根因 = 渐变实心段比真栏边晚 24px 达全浓度（滚动条补偿量误算进渐变段），栏边浓度跳变形成分界——现改为栏边前 8px 即达全浓度。
