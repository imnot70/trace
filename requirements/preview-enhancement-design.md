...
- **页内锚点跳转未生效**（2026-09-09 用户报告，暂缓排查）：`测试用/for_test_01.md` 中 `<a id='tag1'>` 定义锚点、`[jump](#tag1)` 点击无动作。直接原因：2.2 节方案中 `#锚点` 被归入「本期不处理，仅阻止默认」——设计时计划与双链/大纲一起做，未在文档标注为已知限制。待办：页内锚点导航（`#id` 滚动定位）应与 2.4 节待确认项（标题锚点 / markdown-it-anchor）合并进「双链 P1 / 大纲」批次实施；届时同批做 `#` 后的锚点 id 自动补全（用户已提出，见下）。
