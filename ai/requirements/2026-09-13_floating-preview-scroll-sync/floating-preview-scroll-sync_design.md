...
4. 新增 watcher 监听 `app.floatingPreview`：
   - 打开时：`rebindScrollSync()` 绑定到悬浮预览
   - 关闭时：取编辑器当前滚动位置，若固定预览可见则调用 `previewRef.value?.syncToLine()` 同步进度，再 `rebindScrollSync()` 绑回固定预览
...
| 悬浮预览关闭（固定预览可见） | 固定预览自动同步到编辑器当前滚动位置 |
| 悬浮预览关闭（固定预览不可见） | 仅解绑，无额外操作 |
