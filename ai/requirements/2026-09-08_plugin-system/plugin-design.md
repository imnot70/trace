...
- **M3（0.5.0）**：声明式工具栏按钮 + 状态区；类型包发 npm（✅ 已实施；类型包已发布 trace-plugin-api@0.1.0，后续由 CI 自动发布——Trusted Publishing/OIDC，见 guides/plugin-development.md 第 9 节）
- **M4（0.5.x）**：GitHub 索引市场（浏览/安装/更新检查）（✅ 已随 v0.5.0 实施，决策与开发方案见第 13 节）
...
## 13. M4 方案（2026-09-20 确认，已随 v0.5.0 实施）
...
2. ✅ **批次二已实施**：市场 UI（安装/更新/已下架标记）接 M2 管线 + 作者发布指南（`guides/plugin-release.md`）+ PRD FR-2.11.14；实机冒烟全部通过——marketList 真实拉取 ✓、市场安装全链路（下载 → sha256 校验 → 落盘 → 来源登记 → 权限确认 → 激活 → 命令/工具栏可用）✓（2026-09-21，首个收录件 sample 2.2.0）
