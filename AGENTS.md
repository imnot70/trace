...
- 支持 LaTeX 公式、图片粘贴、回收站、收藏/常用、浅色/深色主题与插件系统（v2 M1：进程隔离 + Tier 1 能力 API + 权限确认；M2–M4 见 `requirements/2026-09-08_plugin-system/`）；
...
~~全局搜索~~（已实现）、所见即所得模式、~~图形化冲突解决~~（已实现）、~~定时/变更自动同步~~（已实现）、插件完整 API 与市场（v2 设计已定稿，见 `requirements/2026-09-08_plugin-system/`；M1 已实施，M2–M4 待做）、~~标签~~（已实现）、~~多窗口~~（已作废）、~~导出 PDF/HTML~~（已实现）、~~窗口毛玻璃效果~~（已实现）、~~笔记双链引用~~（P1–P3 已实现）。
