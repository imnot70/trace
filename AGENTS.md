...
~~全局搜索~~（已实现）、所见即所得模式、~~图形化冲突解决~~（已实现）、~~定时/变更自动同步~~（已实现）、插件完整 API 与市场（v2 设计已定稿，见 `requirements/2026-09-08_plugin-system/`，目标 0.4.0）、标签、~~多窗口~~（已作废）、~~导出 PDF/HTML~~（已实现）、~~窗口毛玻璃效果~~（已实现）。近期待办（已排期讨论）：~~文件/文件夹跨库移动~~（已作废）、笔记双链引用（`[[笔记名]]`）。
