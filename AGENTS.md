...
全局搜索、所见即所得模式、图形化冲突解决、定时/变更自动同步、插件完整 API 与市场（v2 设计已定稿，见 `requirements/2026-09-08_plugin-system/`，目标 0.4.0）、标签、~~多窗口~~（已作废）、导出 PDF/HTML、窗口毛玻璃效果。近期待办（已排期讨论）：文件/文件夹移动、笔记双链引用（`[[笔记名]]`）。
