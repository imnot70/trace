...
- 支持 LaTeX 公式、图片粘贴、回收站、收藏/常用、浅色/深色主题与插件系统（v2 M1+M2：进程隔离 + Tier 1 能力 API + 权限确认 + .trace-plugin 导入导出 + 私有存储；M3–M4 见 `requirements/2026-09-08_plugin-system/`）；
...
- 透明窗口底部圆角在浅色壁纸上仍可能显示直角轮廓（**仅 Linux**，推测为系统合成器限制；Windows 已随透明窗口整体禁用而消除；深色壁纸正常），待排查。
...
~~全局搜索~~（已实现）、所见即所得模式、~~图形化冲突解决~~（已实现）、~~定时/变更自动同步~~（已实现）、插件完整 API 与市场（v2 设计已定稿，见 `requirements/2026-09-08_plugin-system/`；M1 + M2 已实施，M3–M4 待做）、~~标签~~（已实现）、~~多窗口~~（已作废）、~~导出 PDF/HTML~~（已实现）、~~窗口毛玻璃效果~~（已实现）、~~笔记双链引用~~（P1–P3 已实现）。
