...
- **分支**：`feat/draft-notes`（基于 main），已推送；隔离实例 CDP 验证通过（2026-09-26 接力会话），待真机验证后合并发 v0.8.8；
- **main** = v0.8.7（tag 指向 `4226953`；09-25 / 09-26 已发 v0.8.3–v0.8.7 六个版本）+ 未发版提交：官网 `site/`（Pages 独立部署，已上线）、**底色纹理**（随 v0.8.8 发布）、suggest 提案池与草稿立项文档；
- **测试基线**：364 项全绿（`npm test`）；typecheck / lint 0 错误（2026-09-26 两次实测确认）。
...
4. **v0.8.6**：FR-2.9.10 心流引用与速览增强（Esc 分级 / `[[` 扁平模糊匹配 / 搜索键盘导航与 Alt+Enter 预览 / 插入引用括号一致性）+ 行插入快捷键（Ctrl+Enter / Ctrl+Shift+Enter）+ 底色过渡；
5. **v0.8.7**：打字机滚动冷却（滚轮不回跳）+ Alt+W 进心流回焦 + Alt+T 打字机开关与状态图标（字母 T）+ 设置页说明文字下移。

> ⚠️ 官网 `site/` 与底色纹理是 v0.8.7 打标签**之后**才合入 main 的，不在任何已发布安装包里：官网经 Pages 独立部署已上线；底色纹理随 v0.8.8 发布（CHANGELOG 该条已从 [0.8.7] 段移回 [未发布]）。

### 待发版（分支 `feat/draft-notes`，基于 main）

6. **v0.8.8 候选**：草稿笔记（FR-2.3.9）——Ctrl+N 写入 userData/scratch 伪库（自动聚焦 / 空草稿复用 / 1s 自动保存），侧栏「草稿」菜单，Ctrl+S 转正选库 / 目录（图片随迁改写引用），搜索集成（徽标「草稿」/ 范围独立勾选 / 增量索引），跨库插入校验，ScratchService + promoteDraft 编排 + 9 项新单测（364 全绿）；另含 main 上的底色纹理。2026-09-26 接力会话完成隔离实例 CDP 验证，并修复 4 个缺陷：草稿无自动聚焦（onMounted 消费时序）、窗口级 Ctrl+S 无草稿分流（G4）、搜索徽标显示 `__scratch__`、创建后菜单不刷新。
...
| # | 事项 | 难度 | 工时 |
|---|------|------|------|
| 1 | 真机手测 feat/draft-notes（CDP 已全覆盖：创建 / 复用 / 自动保存 / 菜单 / 搜索 / 转正含图片 / 重启持久；剩真机手感确认） | — | — |
| 2 | 合并 feat/draft-notes → main → 发 v0.8.8（CHANGELOG 未发布段已就绪，草稿 + 底色纹理同车） | ★ | 0.5h |
...
> 原待办「跨库插入校验」已随草稿功能实现并验证（2026-09-26 销账，见 index 〇节）。
